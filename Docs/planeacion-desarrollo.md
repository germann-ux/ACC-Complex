...
- Sprint 8: Construcción del compilador en línea. En el cliente, `ACC-Compiler/CompiladorACC.razor` envía el código a `ServiceRoots.ACC_COMPILER_Url`. En el backend especializado `ACC.Compiler` (alojado actualmente en `src/API_CompilerACC`), `CompileController` llama a `RoslynCompileService`, que compila en memoria con referencias mínimas a `System.*` y retorna stdout, manteniendo aislamiento de disco y red.

Durante esta fase se mantuvo la maqueta MAUI Blazor que más adelante dará forma a `ACC.MultiPlataform`, centrada en verificar responsividad y carga del iframe de ChatBase.
