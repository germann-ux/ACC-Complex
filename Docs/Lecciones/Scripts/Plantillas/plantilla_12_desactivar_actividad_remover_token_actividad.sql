-- Plantilla 12 - DESACTIVAR ACTIVIDAD + REMOVER TOKEN "actividad"
USE [ACC_Academic];
GO
...
BEGIN TRY
    ...
    IF @OrdenActual IS NULL
        THROW 52027, 'No se encontro la leccion objetivo.', 1;

    IF ISJSON(@OrdenActual) <> 1
        THROW 52028, 'OrdenSecciones actual no es JSON valido.', 1;
    ...
    WHERE CAST([value] AS NVARCHAR(50)) <> N'actividad'
